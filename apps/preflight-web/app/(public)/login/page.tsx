...
export default function LoginPage() {
  ...
  const handleLogin = () => {
    // Use stub auth for testing (dev only)
    signIn(email);
    setStubCookie();
  ...
  const handleOceanheartLogin = () => {
    const returnTo = isDev
      ...
      : "https://preflight.oceanheart.ai/app";
    const authUrl = isDev
      ...
      <div className="relative z-10 max-w-md w-full">
        <div className="bg-zinc-900/50 backdrop-blur-xl rounded-md border border-zinc-800 p-6">
          ...
          <div className="space-y-4">
            {/* Oceanheart Login - Primary in production */}
            <button
              type="button"
            ...
            >
              <span>Sign in with Oceanheart</span>
              <ArrowRight className="w-4 h-4 transition-transform group-hover:translate-x-1" />
            </button>

            {/* Dev-only: Stub auth form */}
            {isDev && (
              <>
                {/* Divider */}
                <div className="relative my-6">
                  ...
                  <div className="relative flex justify-center text-xs">
                    <span className="px-2 bg-zinc-900/50 text-zinc-500">Dev Mode Only</span>
                  </div>
                </div>
                ...
                <form onSubmit={(e) => { e.preventDefault(); handleLogin(); }} className="space-y-4">
                  ...
                  <button
                    type="submit"
                    className="w-full py-2.5 bg-amber-600/20 border border-amber-600/50 text-amber-200 font-medium rounded-md hover:bg-amber-600/30 transition-all duration-200 text-sm"
                  >
                    Sign in (Test Mode)
                  </button>
                </form>
